...
import static akka.FaultTolerance.CounterServiceApi.GetCurrentCount;
import static akka.FaultTolerance.WorkerApi.Do;
import static akka.FaultTolerance.WorkerApi.Start;
...
import akka.FaultTolerance.CounterApi.UseStorage;
import akka.FaultTolerance.CounterServiceApi.CurrentCount;
import akka.FaultTolerance.CounterServiceApi.Increment;
import akka.FaultTolerance.CounterServiceApi.ServiceUnavailable;
import akka.FaultTolerance.StorageApi.Entry;
import akka.FaultTolerance.StorageApi.Get;
import akka.FaultTolerance.StorageApi.StorageException;
import akka.FaultTolerance.StorageApi.Store;
import akka.FaultTolerance.WorkerApi.Progress;
...
public class FaultTolerance {
	...
	* It will continuously notify the sender of the Start message of current Progress.
	...
	public static class Worker extends UntypedActor {
		...
		public void onReceive(Object msg) {
			...
			if (msg.equals(Start) && progressListener == null) {
				...
				getContext().system().scheduler().schedule(Duration.Zero(), Duration.create(1, "second"), getSelf(), Do, getContext().dispatcher(), null);
			} 
		...
		public static class CounterService extends UntypedActor {
			...
			*/
			...
			void forwardOrPlaceInBacklog(Object msg) {
				...
				// Before that we place the messages in a backlog, to be sent to the counter when it is initialized.
				if (counter == null) {
